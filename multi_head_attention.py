# This source code is primarily from https://github.com/keras-team/keras/blob/v2.12.0/keras/layers/attention/multi_head_attention.py#L130-L730
# we just needed to make a couple of adjustments
# in particular, in adition to softmax normalization, we added support for different activations applied to the attention scores
...
class MultiHeadAttention(Layer):
    ...
    def _build_from_signature(self, query, value, key=None):
        ...
        with tf_utils.maybe_init_scope(self):
            ...
            self._build_attention(output_rank, self.activation_type)
    ...
    def _build_attention(self, rank, activation_type):
    ...
    def _masked_softmax(self, attention_scores, attention_mask=None):
        ...
        if attention_mask is not None:
            return self._activation(attention_scores, attention_mask)
        else:
            return self._activation(attention_scores)
    ...
    def call(
        ...
        if use_causal_mask and self.activation_type != 'softmax':
            raise ValueError(f"`use_causal_mask` is set to True, but this is incompatible with `activation_type` {self.activation_type}")
