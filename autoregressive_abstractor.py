...
class AutoregressiveLARSVSA(tf.keras.Model):
    """
    An implementation of an Abstractor-based Transformer module.
    ...
    def __init__(self,
        ...
        # initialize decoder
        self.decoder = MultiAttentionDecoder(**decoder_kwargs, name='decoder')
        self.mha = [HDSymoblicAttention(1024,160) for _ in range(2)]
        # initialize final prediction layer
        self.final_layer = layers.Dense(output_dim, name='final_layer')
        self.bn1 = layers.BatchNormalization(synchronized=True)
        self.bn2 = layers.BatchNormalization(synchronized=True)
        #self.fc1 = [layers.Dense(64, name='final_layer',activation='swish',trainable=True,kernel_initializer="glorot_uniform") for _ in range(2)]
    ...
    def call(self, inputs):
        ...
        source = self.pos_embedding_adder_input(source)
        x = self.bn1(source)
        h0 = tf.reshape(self.mha[0](x),(-1,160,16,64))
        h1 = tf.reshape(self.mha[1](x),(-1,160,16,64))
        abstracted_context = tf.reduce_sum(h0,axis=2) + tf.reduce_sum(h1,axis=2) 

        abstracted_context = self.bn2(abstracted_context)
        # embed target and add positional embedding
        target_embedding = self.target_embedder(target)
        target_embedding = self.pos_embedding_adder_target(target_embedding)
        ...
        decoder_inputs = [target_embedding, x, abstracted_context]
